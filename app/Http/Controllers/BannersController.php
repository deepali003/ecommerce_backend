...
// 			$banner->link = $data['link'];
        ...
        //             $banner_path = 'images/backend_images/banner/'.$fileName;
     			// 	Image::make($image_tmp)->resize(1140, 340)->save($banner_path);
     			// 	$banner->image = 'http://127.0.0.1:8000/images/backend_images/banner'.'/'.$fileName;  
     				
     				 $fileName = rand(111,99999).'.'.$extension;
                    $banner_path = HelperController::GETBannerUrl($fileName);
     				Image::make($image_tmp)->resize(1140, 340)->save($banner_path);
     				$banner->image = HelperController::GETCompleteBannerUrl($fileName); 
                }
            }
        ...
        if($request->isMethod('post')){
            ...
            if($request->hasFile('image')){
                ...
                if ($image_tmp->isValid()) {
                    ...
                    $extension = $image_tmp->getClientOriginalExtension();
                    $filename = rand(111,99999).'.'.$extension;
                    $fileName = HelperController::GETCompleteBannerUrl($filename);
                    $banner_path = HelperController::GETBannerUrl($filename);
                    Image::make($image_tmp)->resize(1140, 340)->save($banner_path); 
                }
