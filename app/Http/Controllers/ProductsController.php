...
class ProductsController extends Controller
{
		...
		if($request->isMethod('post')){
            ...
            if($request->hasFile('image')){
                ...
                if ($image_tmp->isValid()) {
                    ...
                    $large_image_path = HelperController::GETProductUrl('large'.'/'.$fileName);
                    $medium_image_path = HelperController::GETProductUrl('medium'.'/'.$fileName);  
                    $small_image_path =  HelperController::GETProductUrl('small'.'/'.$fileName);   

	                Image::make($image_tmp)->save($large_image_path);
 					Image::make($image_tmp)->resize(600, 600)->save($medium_image_path);
     				Image::make($image_tmp)->resize(300, 300)->save($small_image_path);
                    $product->image  = HelperController::GETCompleteProductUrl($fileName);
     			// 	$product->image = 'http://127.0.0.1:8000/images/backend_images/product/large'.'/'.$fileName; 
		...
		if($request->isMethod('post')){
            ...
            if($request->hasFile('image')){
                ...
                if ($image_tmp->isValid()) {
                    ...
                    $extension = $image_tmp->getClientOriginalExtension();
                    $filename = rand(111,99999).'.'.$extension;
                    // $fileName = 'http://127.0.0.1:8000/images/backend_images/product/medium'.'/'.$filename;
                    $fileName = HelperController::GETCompleteProductUrl($filename); 
    ...
    public function viewOrderDetails($order_id){
        ...
        $userDetails = User::where('userid',$user_id)->first();
