...
class UsersController extends Controller
{
    ...
    public function importUsersList()
    {
        ...
        return view('admin/users/add_import');
    }
    public function importUsers(Request $request){
       
        // $data = Excel::toArray(new UsersImport, request()->file('file')->getRealPath(), null, \Maatwebsite\Excel\Excel::XLSX);      
        $request->validate([
            'file' => 'required'
        ]);
       
        $fetcdata = User::get()->count();
        if($fetcdata){
            return redirect()->back()->with('flash_message_error','Data already exists!');
        }else{
            $data = Excel::import(new UsersImport, request()->file('file'));
            return redirect()->back()->with('flash_message_success','Data imported successfully!');
        }
